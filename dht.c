/*
=== DHT-11 (temperature, humidity) timer over ESP8266 wifi ===
...
#define SSID_CHANNEL 0 //optional channel for AP scan
#define MIN_SCANTIME 1000 //wifi min scan time in ms
#define MAX_SCANTIME 15000 //wifi max scan time in ms
#define HOSTNAME "dht11"
#define NTP0 "0.pool.ntp.org"
#define NTP1 "1.pool.ntp.org"
#define NTP2 "2.pool.ntp.org"
#define DNS "192.168.1.1"
//GPIO2 is wired to DHT11
//At and after reset, it is weakly pulled up (wpu)
//At reset, its ouput is disabled (oe=0)
#define GPIO_NO 2
//default function0 of GPIO2
#define INSTANCE_NAME PERIPHS_IO_MUX_GPIO2_U
//default I/O/T function
#define GPIO_FUNC FUNC_GPIO2
...
#define TIMER_CRONLINE "0-4,20-24,40-44 * * * * *" //sec min hour day month day-of-week
#define TIMER_ARM_INTERVAL 5000 //period in ms to get DHT data
								                //set it based on timer schedule
#define TIMEZONE 12
...
TIMEZONE
*/
#define WEB_SERVER_PATH "/dht-timer-parameters.txt"
#define WEB_SERVER_PATH2 "/dht11.php?"
...
#define TIMEOUT 150 //us - timeout waiting to a DHT pulse
#define DELAY 1 //us - DHT loop delay
...
os_timer_t timer, esp_config, http_update, process_data;

int timer_arm_interval = TIMER_ARM_INTERVAL;
int tz = TIMEZONE;
uint8 seconds[60], minutes[60], hours[24], days[31], months[12], dweek[7], bit_number;
uint32 timex, pulse_length[41];
...
uint8 cron_parser(char *);

void read_sensor();
void print_error(uint8);
...
void ICACHE_FLASH_ATTR process_data_cb() {
	//os_printf("process_data_cb called\n");
	os_timer_disarm(&process_data);
	uint8 i;
	uint16 th = 0, tl = 0, hh = 0, hl = 0, cs = 0;
	uint32 ack_response_time = 80; //us
	uint32 deviation = 20; //us
	uint32 median = 50; //us
	char s[16];

	//process data
	//ack length must be 80us +/- 20us
	if (pulse_length[0] == 0 || pulse_length[0] < ack_response_time - deviation || pulse_length[0] > ack_response_time + deviation)
		os_printf("no response: missing 80us +/- 20us low pulse!\n");

	for (i = 1; i < 9; i++) {
		hh <<= 1;
		if (pulse_length[i] > median) hh |= 1;
		else if (pulse_length[i] == 0) print_error(i);
	}
	//os_printf("hh %u\n", hh);
	
	for (i = 9; i < 17; i++) {
		hl <<= 1;
		if (pulse_length[i] > median) hl |= 1;
		else if (pulse_length[i] == 0) print_error(i);
	}
	//os_printf("hl %u\n", hl);
...
void ICACHE_FLASH_ATTR user_init(void) {
	...
	//must be called at boot up according to gpio.h
	gpio_init();
	uint32 gpio_reg = gpio_register_get(INSTANCE_NAME);
	uint32 wpu = (gpio_reg >> 7) & 1;
	os_printf("default gpio func no %u, pull-up %u, gpio_reg %u\n", ((gpio_reg >> 6) & 4) | ((gpio_reg >> 4) & 3), wpu, gpio_reg);
	if (!wpu) {
		os_printf("dis out\n");
		GPIO_DIS_OUTPUT(GPIO_NO);
		os_printf("en wpu\n");
		PIN_PULLUP_EN(INSTANCE_NAME);
	}
...
void ICACHE_FLASH_ATTR esp_config_cb(void *arg) {
	...
	if (wifi_station_get_connect_status() == STATION_GOT_IP) {
		...
		char par[os_strlen(TIMER_CRONLINE) + 15];
		os_sprintf(par, "%s\n%u\n%d\n", TIMER_CRONLINE, TIMER_ARM_INTERVAL, TIMEZONE);
		parse_body(par);

		if (HTTP_UPDATE) http_update_cb(NULL);
		else os_timer_arm(&timer, timer_arm_interval, 0);
	}
...
void ICACHE_FLASH_ATTR timer_cb(void *arg) {
	...
	if (current_stamp > 0) {
		...
		i = 0;
		if (!datetime_field[3]) {
			os_printf("timer: hms is NULL - datetime is invalid\n");
			return;
		}
		...
		for (i = 0; i <= 3; i++) {
			...
			case 0: //day-of-the-week
				if (!datetime_field[i]) {
					os_printf("timer: day-of-week is NULL\n");
					return;
				}
				if (os_strncmp(datetime_field[i], "Sun", 3) == 0) j = 0;
					...
					os_printf("timer: invalid day-of-week\n");
					return;
				}
				if (!dweek[j]) on_off = false;
				break;
			case 1: //Month
				if (!datetime_field[i]) {
					os_printf("timer: month is NULL\n");
					return;
				}
				if (os_strncmp(datetime_field[i], "Jan", 3) == 0) j = 0;
					...
					os_printf("timer: invalid month\n");
					return;
				}
				if (!months[j]) on_off = false;
				break;
			case 2: //day
				if (!datetime_field[i]) {
					os_printf("timer: day is NULL\n");
					return;
				}
				if (isdigit(datetime_field[i][0]))
					j = atoi(datetime_field[i]) - 1;
				else {
					os_printf("timer: days is not a number\n");
					return;
				}
				if (j > 30) {
					os_printf("timer: days are from 1 to 31\n"); 
					return;
				}
				if (!days[j])	on_off = false;
				break;
			case 3: //hms
				//hour
				if (!hms[0]) {
					os_printf("timer: hours is NULL\n");
					return;
				}
				if (isdigit(hms[0][0]))
					j = atoi(hms[0]);
				else {
					os_printf("timer: hours is not a number\n"); return;
				}
				if (j > 23) {
					os_printf("timer: hours are from 0 to 23\n"); return;
				}
				if (!hours[j]) on_off = false;

				//minute
				if (!hms[1]) {
					os_printf("timer: minutes is NULL\n");
					return;
				}
				if (isdigit(hms[1][0]))
					j = atoi(hms[1]);
				else {
					os_printf("timer: minutes is not a number\n"); 
					return;
				}
				if (j > 59) {
					os_printf("timer: minutes are from 0 to 59\n"); 
					return;
				}
				if (!minutes[j]) on_off = false;

				//seconds
				if (!hms[2]) {
					os_printf("timer: seconds is NULL\n");
					return;
				}
				if (isdigit(hms[2][0]))
					j = atoi(hms[2]);
				else {
					os_printf("timer: seconds is not a number\n"); 
					return;
				}
				if (j > 59) {
					os_printf("timer: seconds are from 0 to 59\n"); 
					return;
				}
				if (!seconds[j]) on_off = false;
				break;
			}
			if (!on_off) break;
		}
		if (on_off) {
			os_printf("reading sensor...\n");
			read_sensor();
		}
	}
	else {
	...
	os_timer_arm(&timer, timer_arm_interval, 0);
}
void ICACHE_FLASH_ATTR http_update_cb(void * arg) {
	...
	if (arg) {
		unsigned int len = os_strlen((char *)arg);
		//os_printf("len %u\n", len);
		con->reverse = (char *)os_zalloc(len + 1);
		os_memcpy(con->reverse, arg, len);
		//os_printf("arg %s\n", (char *)con->reverse);
	}
...
void ICACHE_FLASH_ATTR connect_cb(void * arg) {
	...
	char sdk_version[32];
	char web_server_path[64];
	os_sprintf(sdk_version, "%s", system_get_sdk_version());
	if (con->reverse)
		os_sprintf(web_server_path, "%s%s", WEB_SERVER_PATH2, (char *)con->reverse);
	else os_sprintf(web_server_path, "%s", WEB_SERVER_PATH);
...
void ICACHE_FLASH_ATTR disconnect_cb(void * args) {
	...
	if (con) {
		...
		if (con->reverse) os_free(con->reverse);
		os_free(con);
	}
}
...
uint8 ICACHE_FLASH_ATTR parse_body(char * body) {
	char * cronline = NULL;
	char * timer_arm_interval_char = NULL;
	char * timezone = NULL;
	uint8 e;
	os_memset(seconds, 0, 60);
	os_memset(minutes, 0, 60);
	os_memset(hours, 0, 24);
	os_memset(days, 0, 31);
	os_memset(months, 0, 12);
//	os_printf("body:\n%s\n", body);

	cronline = strtok(body, "\r\n");
	if (!cronline) return 1;
//	os_printf("cronline %s\n", cronline);
	timer_arm_interval_char = strtok(NULL, "\r\n");
	if (!timer_arm_interval_char) return 3;
//	os_printf("timer_arm_interval_char %s\n", timer_arm_interval_char);
	if (isdigit(timer_arm_interval_char[0])) 
		timer_arm_interval = atoi(timer_arm_interval_char);
	else return 4;
...
//	os_printf("timezone %s\n", timezone);
	...
	e = cron_parser(cronline);
	if (e != 0) os_printf("cron_parser(cronline) error %u\n", e);
}
void ICACHE_FLASH_ATTR receive_cb(void * args, char * buf, unsigned short len) {
	...
	if (buf[0] != '\0') {
		...
		if (os_strncmp(buf, HTTP_VERSION, os_strlen(HTTP_VERSION)) == 0) {
			...
			if (http_status == 200) {
				body = os_strstr(buf, "\r\n\r\n") + 4;
				if (os_strlen(body) == content_len && !con->reverse)
					parse_body(body);
...
void ICACHE_FLASH_ATTR configure_timers(void) {
	...
	os_timer_setfn(&process_data, (os_timer_func_t *)process_data_cb, NULL);
}

uint8 ICACHE_FLASH_ATTR cron_parser(char * cronline) {
...
void read_sensor() {
	...
	//open the bus
	PIN_PULLUP_DIS(INSTANCE_NAME);

	//send request
	GPIO_OUTPUT_SET(GPIO_ID_PIN(GPIO_NO), 0);
	...
	//enable input
	GPIO_DIS_OUTPUT(GPIO_ID_PIN(GPIO_NO));
	...
	for (i = 0; i < 41; i++) {
		timeout = 0;
		while (GPIO_INPUT_GET(GPIO_NO) == 0 && timeout++ < TIMEOUT) {
			os_delay_us(DELAY);
		}
		timeout = 0;
		timex = system_get_time();
		while (GPIO_INPUT_GET(GPIO_NO) == 1 && timeout++ < TIMEOUT) {
			os_delay_us(DELAY);
		}
		pulse_length[i] = system_get_time() - timex;
	}
	os_timer_arm(&process_data, 1000, 0);
}
void ICACHE_FLASH_ATTR print_error(uint8 i) {
	os_printf("error: missing data bit %u\n", i);
}
